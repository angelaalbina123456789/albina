...
int main() {
	...
	if(size != 14) {
		printf("Не удалось заисать 14 байт в файл\n");
		exit(-1);
	}
	else printf ("Строка записана в файл \n");
	if (close(fd) < 0) {
		printf("Не получилочь закрыть файл\n");
		exit(-1);
	}
	char resstring[14];
	if((fd = open("wrfile.txt", O_RDONLY)) < 0) {
		printf("Ошибка пр открытии файла на чтение\n");
		exit(-1);
	}
	...
	else printf("Прочитанная строка %s/n", resstring);
	if (close(fd) < 0) {
